        ...
        register_instance_option :controller do
          Proc.new do # Configurando PDF
            ...
            Prawn::Document.new(PDF_OPTIONS) do |pdf|
              ...
              pdf.text "Informe del Representante", :size => 32, :style => :bold, :align => :center
              pdf.move_down 80
              ...
              if @object.status
                pdf.text "Estado: #{@object.status}", :size => 12, :align => :justify, :inline_format => true
                pdf.move_down 8
              end

              if @object.email
                pdf.text "Correo electrónico: #{@object.email}", :size => 12, :align => :justify, :inline_format => true
                pdf.move_down 8
              end
              ...
              if @object.comissions.where(status: :pending).count > 0
                ...
                pdf.text "comissiones Pendientes", :size => 14, :align => :justify, :inline_format => true, :style => :bold
                pdf.move_down 14

                total = 0

                @object.comissions.where(status: :pending).each do |c|
                  pdf.text "Id #{c.id}, valor R$#{c.value}, generada em #{c.created_at.strftime("%d/%m/%y as %H:%M")}",
              ...
              if @object.clients.count > 0
                ...
                pdf.text "Clientes Activos", :size => 14, :align => :justify, :inline_format => true, :style => :bold
                pdf.move_down 14
                ...
                @object.clients.each do |c|
                  ...
                  pdf.text "La empresa: #{c.company_name}",
                    :size => 12, :align => :justify, :inline_format => true
                  pdf.move_down 8
              ...
              pdf.text "Enlace al manual de Prawn clicable", :size => 10, :inline_format => true, :valign => :bottom, :align => :left
              # Inclui em baixo da folha do lado direito a data e o némero da página usando a tag page
              pdf.number_pages "Generado: #{(Time.now).strftime("%d/%m/%y as %H:%M")} - Página ", :start_count_at => 0, :page_filter => :all, :at => [pdf.bounds.right - 140, 7], :align => :right, :size => 8
